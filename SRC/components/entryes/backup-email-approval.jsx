...
function BackupEmailApproval() {
  ...
  useEffect(() => {
    ...
    handleBackupEmailAction();
  }, [token, actionParam]);

  const handleBackupEmailAction = async () => {
    ...
    try {
      ...
      const endpoint = actionParam === 'approve' 
        ? `${apiUrl}/auth/backup-email/approve`
        : `${apiUrl}/auth/backup-email/block`;
      
      console.log('Backup Email Approval: Отправка запроса на', endpoint, 'с токеном', token);
      ...
      console.log('Backup Email Approval Response:', response.data);

      if (response.data.success) {
        setStatus('success');
        if (actionParam === 'approve') {
          setMessage('Ваша резервная почта успешно добавлена');
        } else {
          setMessage('Добавление резервной почты отклонено');
        }
      } else {
        setStatus('error');
        setMessage(actionParam === 'approve' 
          ? 'Ваша резервная почта не была добавлена' 
          : 'Ошибка отклонения добавления резервной почты');
        setError(response.data.message || response.data.error || 'Неизвестная ошибка');
      }
    } catch (error) {
      console.error('Ошибка обработки резервной почты:', error);
      setStatus('error');
      setMessage(actionParam === 'approve' 
        ? 'Ваша резервная почта не была добавлена' 
        : 'Ошибка отклонения добавления резервной почты');
...
export default BackupEmailApproval;
