...
const SYMBOL = "USD";
const CANDLES_TO_DISPLAY = 280; // Количество отображаемых свечей
...
function drawCandle(candle) {
  ...
  ctx.fillRect(candle.x, candleTop, candle.width, candleHeight);
...
function animate() {
  try {
    // Очистка с прозрачностью вместо белого фона
    ctx.clearRect(0, 0, canvas.width, canvas.height);

    // Обновляем масштаб перед отрисовкой
    updateScaleAndOffset();

    // Отрисовка сетки (полупрозрачная)
    ...
    if (candles.length > 0 && candles[0].x + config.candleWidth < 0) {
      ...
      if (historicalData.length > 0) {
        const nextIndex =
          (historicalData.findIndex((d) => d.date === lastCandle.date) + 1) %
